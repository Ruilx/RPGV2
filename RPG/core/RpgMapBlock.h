 ...
 *
 * 2018/09/14: 关于优先级的解释:
 * 优先级如上图, ZValue比较高就是优先级比较高, 优先级为qreal, 但一般使用的是int整数层, 一层占用的位置是0.0f-0.9*f, 所以定义
 * 0.0f - 0.39*f: ZValue值比较低的显示在玩家后面
 * 0.4f - 0.49*f: ZValue值为玩家精灵的位置, 其中主角的位置为0.490f, 其他角色均小于这个值
 * 0.5f - 0.99*f: ZValue值比较高的显示在玩家前面
 */
